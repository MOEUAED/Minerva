...
<body class="bg-slate-100 min-h-screen flex">
    ...
    <main class="flex-1 overflow-y-auto p-8">
        <header class="flex justify-between items-center mb-10">
            ...
            <div class="flex space-x-4">
                <button onclick="openModal('classModal')" class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl shadow-lg transition-all flex items-center space-x-2">
            ...
            <section class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
                ...
                <form action="/teacher/addStudent" method="POST" class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="fullname" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Email</label>
                        <input type="email" name="email" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none transition-all">
                    </div>
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Classe</label>
                        <select name="class_id" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:ring-2 focus:ring-blue-500 outline-none bg-white">
    ...
    <div id="classModal" class="fixed inset-0 bg-black/50 hidden flex items-center justify-center p-4">
        <div class="bg-white rounded-2xl p-8 max-w-sm w-full">
            ...
            <input type="text" placeholder="Nom de la classe (ex: Terminale A)" class="w-full px-4 py-2.5 rounded-xl border mb-4 outline-none focus:ring-2 focus:ring-blue-500">
            <div class="flex space-x-3">
                <button onclick="closeModal('classModal')" class="flex-1 py-2 bg-slate-100 rounded-xl font-bold">Annuler</button>
                <button class="flex-1 py-2 bg-blue-600 text-white rounded-xl font-bold shadow-lg shadow-blue-200">Créer</button>
            </div>
        </div>
    </div>
